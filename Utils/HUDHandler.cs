...
internal class HUDHandler : MonoBehaviour
{
    ...
    public void StartPlayerRevivalCountDown()
    {
        if (!isRunning && !isPermaDead)
        {
            ...
            {
                text.text = EditText(LCAutoRevive.outOfRevivesText, false);
                foreach (PlayerControllerB player in StartOfRound.Instance.allPlayerScripts)
                {
    ...
    private IEnumerator WaitForPlayerRevival()
    {
        ...
        while (timeLeft >= 0f)
        {
            ...
            text.text = EditText(LCAutoRevive.reviveTimerText, true, Mathf.CeilToInt(timeLeft).ToString());
        }
        canRevive = true;
        if (InputUtilsCompat.Enabled && InputUtilsCompat.ReviveKey != null && LCAutoRevive.waitForInput)
        {
            text.text = EditText(LCAutoRevive.waitingForInputText, true, InputUtilsCompat.ReviveKey.controls[0].displayName);
        }
        else
        {
            text.text = EditText(LCAutoRevive.revivingNowText, true);
            if (!StartOfRound.Instance.shipIsLeaving && !StartOfRound.Instance.inShipPhase)
            {
                foreach (PlayerControllerB player in StartOfRound.Instance.allPlayerScripts)
                {
    ...
    internal string EditText(string s, bool showRevives, string value = "")
    {
        string newText = s;
        newText = newText.Replace(LCAutoRevive.replacementSymbol, value);
